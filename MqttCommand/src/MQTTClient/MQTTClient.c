...
void printPublishMessage(MQTTMessage *message){
printf("%d,%.*s----qos:%d----dup:%d\n", message->id,(int)message->payloadlen, (char*)message->payload,message->qos,message->dup);
}
...
exit:
    c->ping_outstanding += (c->ping_outstanding>0);
//    printf("keepAlive ----- %d\n",c->ping_outstanding);
    return rc;
}
